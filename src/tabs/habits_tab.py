...
class HabitsTab(ttk.Frame):
    ...
    def setup_ui(self):
        ...
        ttk.Label(title_frame, text="Привычки", font=("Arial", 16, "bold")).pack(side=tk.LEFT)

        self.compact_mode = tk.BooleanVar(value=False)
        ttk.Checkbutton(title_frame, text="Компактный режим", variable=self.compact_mode, command=self.toggle_compact_mode, takefocus=0).pack(side=tk.LEFT, padx=10)

        # Добавляем кнопку статистики
        ttk.Button(title_frame, text="Статистика", style="Secondary.TButton", command=self.show_stats, takefocus=0).pack(side=tk.LEFT, padx=10)
        
        # Добавляем кнопку сброса всех отметок привычек
        ttk.Button(title_frame, text="Сбросить все отметки", style="Secondary.TButton", command=self.reset_all_habits, takefocus=0).pack(side=tk.LEFT, padx=10)
        ...
        ttk.Button(buttons_frame, text="+ Добавить время", style="Secondary.TButton", command=self.add_custom_time, takefocus=0).pack(side=tk.LEFT, padx=5)
        ...
        self.canvas = tk.Canvas(canvas_container, bg=bg_color, highlightthickness=0, bd=0)
        ...
        self.canvas_frame = self.canvas.create_window((0, 0), window=self.times_frame, anchor="nw")
    ...
    def create_time_frame(self, parent, time_name):
        ...
        if not self.compact_mode.get():
            if time_name not in self.default_times:
                ttk.Button(control_frame, text="🗑", width=3, command=lambda: self.remove_time(time_name), takefocus=0).pack(side=tk.RIGHT, padx=2)

            ttk.Button(control_frame, text="↑", width=3, command=lambda: self.move_time(time_name, -1), takefocus=0).pack(side=tk.LEFT, padx=2)
            ttk.Button(control_frame, text="↓", width=3, command=lambda: self.move_time(time_name, 1), takefocus=0).pack(side=tk.LEFT, padx=2)
            
            timer_settings_frame = ttk.Frame(control_frame)
            timer_settings_frame.pack(side=tk.RIGHT, padx=(10, 0))
            
            ttk.Label(timer_settings_frame, text="Быстрый таймер (мин):").pack(side=tk.LEFT)
            
            quick_timer_var = tk.StringVar(value=str(self.time_settings.get(time_name, {}).get("quick_timer_minutes", "")) 
                                          if self.time_settings.get(time_name, {}).get("quick_timer_minutes") is not None else "")
            
            quick_timer_entry = ttk.Spinbox(timer_settings_frame, from_=1, to=180, width=5, textvariable=quick_timer_var)
            ...
            def save_quick_timer_minutes(event=None):
                try:
                    ...
                    if value:
                        self.time_settings[time_name]["quick_timer_minutes"] = int(value)
                    else:
        ...
        if not self.compact_mode.get():
            ...
            ttk.Button(add_frame, text="Добавить привычку", command=lambda: self.add_habit(time_name, entry), takefocus=0).pack(side=tk.LEFT, padx=5)
        ...
        timer_button = ttk.Button(buttons_right_frame, text="⏱ Таймер", command=lambda: self.start_timer_for_time(time_name), takefocus=0)
        timer_button.pack(side=tk.LEFT, padx=5)
        
        quick_timer_minutes = self.time_settings.get(time_name, {}).get("quick_timer_minutes")
        if quick_timer_minutes is not None:
            quick_timer_button = ttk.Button(buttons_right_frame, text=f"⏱ {quick_timer_minutes} мин", command=lambda minutes=quick_timer_minutes, name=time_name: self.create_quick_timer(name, minutes), takefocus=0)
        ...
        for habit in self.habits[time_name]:
            ...
            if not self.compact_mode.get():
                ...
                ttk.Checkbutton(
                    ...
                    command=lambda h=habit, v=enabled_var: self.toggle_habit(h, v.get()),
                    takefocus=0
            ...
            for i in range(max_visible_repeats):
                ...
                checkbox = ttk.Checkbutton(
                    ...
                    command=lambda h=habit, index=i, var=checkbox_var: self.toggle_repeat(h, index, var.get()),
            ...
            notify_btn = ttk.Button(
                ...
                width=3,
                command=lambda h=habit, v=notifications_var: self.toggle_notifications(h, v),
                takefocus=0
            )
            ...
            if habit.get("notifications", True):
                ...
                time_text = f"{habit.get('start_time', '')} - {habit.get('end_time', '')}"
                if info_text:
    ...
    def edit_habit(self, habit, time_name):
        ...
        repeats_entry = ttk.Spinbox(
            repeats_frame, from_=1, to=100, width=5, font=("Segoe UI", 11), textvariable=repeats_var
        )
        ...
        # Комментарий
        ttk.Label(main_frame, text="Комментарий (необязательно):", font=("Segoe UI", 11)).pack(
            anchor=tk.W, pady=(10, 5)
        )
        
        comment_text = tk.Text(main_frame, height=4, width=40, wrap=tk.WORD, font=("Segoe UI", 11))
        if habit.get("comment"):
        ...
        def save_edited_habit():
            ...
            for h in self.habits.get(time_name, []):
                if h["name"] == name and h != habit:
                    messagebox.showwarning("Ошибка", f"Привычка '{name}' уже существует")
                    return
        ...
        ttk.Button(
            btn_frame, text="Отмена", style="Secondary.TButton", command=dialog.destroy, width=15
        ).pack(side=tk.RIGHT, padx=(10, 0))

        ttk.Button(
            btn_frame, text="Сохранить", style="Accent.TButton", command=save_edited_habit, width=15
        ).pack(side=tk.RIGHT)
    ...
    def add_habit_dialog(self, time_name, habit_name):
        ...
        ttk.Button(
            btn_frame, text="Сохранить", style="Accent.TButton", command=save_habit, width=15
        ).pack(side=tk.RIGHT)
    ...
    def remove_time(self, time_name):
        ...
        if messagebox.askyesno("Подтверждение", f"Удалить '{time_name}' и все привычки в нем?"):
    ...
    def start_timer_for_time(self, time_name):
        ...
        ttk.Label(main_frame, text="Установите таймер:", font=("Segoe UI", 12, "bold")).pack(
            anchor=tk.CENTER, pady=(0, 15)
        )
        ...
        hour_spinner = ttk.Spinbox(
            time_frame, from_=0, to=23, width=3, textvariable=hour_var, font=("Segoe UI", 11)
        )
        ...
        minute_spinner = ttk.Spinbox(
            time_frame, from_=0, to=59, width=3, textvariable=minute_var, font=("Segoe UI", 11)
        )
        ...
        second_spinner = ttk.Spinbox(
            time_frame, from_=0, to=59, width=3, textvariable=second_var, font=("Segoe UI", 11)
        )
        ...
        for text, minutes in preset_buttons:
            ttk.Button(
                presets_frame,
                text=text,
                command=lambda m=minutes: (hour_var.set(str(m // 60)), minute_var.set(str(m % 60)), second_var.set("0")),
        ...
        ttk.Button(
            buttons_frame, text="Отмена", style="Secondary.TButton", command=dialog.destroy
        ).pack(side=tk.RIGHT, padx=(10, 0))

        ttk.Button(
            buttons_frame, text="Запустить", style="Accent.TButton", command=start_custom_timer
        ).pack(side=tk.RIGHT)
    ...
    def create_time_timer(self, time_name, minutes, dialog=None):
        ...
        if hasattr(self, 'parent') and hasattr(self.parent, 'timers_tab'):
            timer_name = f"Привычки: {time_name}"
            self.parent.timers_tab.add_timer(timer_name, minutes)
            self.parent.notebook.select(self.parent.timers_tab_index)
            self.show_toast_notification(f"Таймер для '{time_name}' запущен на {minutes} мин")
        else:
            messagebox.showinfo("Информация", f"Таймер для '{time_name}' на {minutes} мин")
                ...
                try:
                    ...
                    if isinstance(data, list):
                        ...
                        if general_time not in self.all_times:
                            ...
                            self.time_settings[general_time] = {"quick_timer_minutes": None}
                        
                        for habit_data in old_habits:
                            if not all(key in habit_data for key in ["name", "interval", "start_time", "end_time", "enabled"]):
                                print(f"Пропущена привычка с неполными данными: {habit_data}")
                                continue

                            if "last_reminder" in habit_data and habit_data["last_reminder"]:
                                try:
                                    habit_data["last_reminder"] = datetime.fromisoformat(habit_data["last_reminder"])
                                except ValueError:
                    ...
                    for time_name, habits_list in self.habits.items():
                        for habit in habits_list:
                            if habit.get("completed", False) and habit.get("completed_time"):
                                try:
                                    completed_time = datetime.fromisoformat(habit["completed_time"])
    ...
    def show_stats(self):
        ...
        main_frame = ttk.Frame(self.stats_window, padding="20 20 20 20")  # Увеличенные отступы
        main_frame.pack(fill=tk.BOTH, expand=True)
        
        ttk.Label(main_frame, text="Статистика привычек", font=("Segoe UI", 18, "bold")).pack(pady=(0, 20))  # Увеличенный шрифт и отступы
            ...
            for habit in habits_list:
                if habit.get("enabled", True):
                    ...
                    if habit.get("completed", False):
                        ...
                        time_stats[time_name]['completed_repeats'] += habit.get("completed_repeats", 0)
                        completed_repeats += habit.get("completed_repeats", 0)
        
        # Отображаем общую статистику
        ttk.Label(stats_frame, text="Общая статистика:", font=("Segoe UI", 14, "bold")).pack(anchor=tk.W, pady=(0, 10))  # Увеличенный шрифт и отступы
        
        if total_habits > 0:
            completion_percentage = int((completed_habits / total_habits) * 100)
            repeats_percentage = int((completed_repeats / total_repeats) * 100) if total_repeats > 0 else 0
            
            ttk.Label(stats_frame, 
                     text=f"Выполнено {completed_habits} из {total_habits} привычек ({completion_percentage}%)",
                     font=("Segoe UI", 12)).pack(anchor=tk.W)  # Увеличенный шрифт
            
            ttk.Label(stats_frame, 
                     text=f"Выполнено {completed_repeats} из {total_repeats} повторений ({repeats_percentage}%)",
                     font=("Segoe UI", 12)).pack(anchor=tk.W, pady=(0, 15))  # Увеличенный шрифт и отступы
        else:
            ttk.Label(stats_frame, text="Нет активных привычек", font=("Segoe UI", 12)).pack(anchor=tk.W, pady=(0, 15))  # Увеличенный шрифт и отступы
        
        # Отображаем статистику по времени дня
        ttk.Label(stats_frame, text="Статистика по времени дня:", font=("Segoe UI", 14, "bold")).pack(anchor=tk.W, pady=(5, 10))  # Увеличенный шрифт и отступы
        
        for time_name in self.all_times:
            if time_name in time_stats and time_stats[time_name]['total'] > 0:
                time_completion = int((time_stats[time_name]['completed'] / time_stats[time_name]['total']) * 100)
                time_repeats = int((time_stats[time_name]['completed_repeats'] / time_stats[time_name]['total_repeats']) * 100) if time_stats[time_name]['total_repeats'] > 0 else 0
                ...
                ttk.Label(stats_frame, 
                         ...
                         font=("Segoe UI", 11)).pack(anchor=tk.W, pady=(0, 5))  # Увеличенный шрифт и отступы
        
        # Добавляем кнопку закрытия с увеличенным размером
        ttk.Button(main_frame, text="Закрыть", command=self.stats_window.destroy, width=15, style="Accent.TButton").pack(pady=(20, 0))
    ...
    def add_comment(self, habit):
        ...
        ttk.Label(
            ...
            font=("Segoe UI", 11)
        ).pack(anchor=tk.W, pady=(0, 15))  # Увеличенные отступы
        
        comment_text = tk.Text(main_frame, height=6, width=45, wrap=tk.WORD, font=("Segoe UI", 11))  # Увеличенные размеры текстового поля
        if habit.get("comment"):
            comment_text.insert("1.0", habit["comment"])
        comment_text.pack(fill=tk.BOTH, expand=True, pady=(0, 15))  # Увеличенные отступы
        self.add_copy_paste_menu(comment_text)
        ...
        # Увеличенные размеры кнопок
        ttk.Button(
            btn_frame, text="Пропустить", style="Secondary.TButton", command=skip_comment, width=15
        ).pack(side=tk.RIGHT, padx=(10, 0))
        
        ttk.Button(
            btn_frame, text="Сохранить", style="Accent.TButton", command=save_comment, width=15
        ).pack(side=tk.RIGHT)
    ...
    def update_habit_button_style(self, button, habit):
        ...
        if habit.get("repeats", 1) > 1:
            button.config(text=f"{habit['name']} ({habit.get('completed_repeats', 0)}/{habit.get('repeats', 1)})")
        else:
    ...
    def reset_all_habits(self):
        ...
        if messagebox.askyesno("Подтверждение", "Сбросить все отметки выполнения привычек и комментарии?"):
            for time_name, habits_list in self.habits.items():
                for habit in habits_list:
                    habit["completed"] = False
                    habit["completed_repeats"] = 0
            ...
            self.update_times_display()
            self.update_stats_display()
            messagebox.showinfo("Выполнено", "Все отметки привычек и комментарии сброшены")
