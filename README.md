...
- 📱 **Telegram интеграция** — управление сообщениями из приложения
...
- Фильтрация по папкам (например, "Архив")
- Безопасная авторизация через API
- Сохранение сессии
...
Приложение поддерживает 20+ тем через меню **"Настройки" → "Тема"**:
- ubuntu (по умолчанию)
- arc
- equilux
- adapta
...
│   │   └── telegram_tab.py  # Telegram интеграция
...
#### 3. Безопасность Telegram API 🟡
- **Где**: `telegram_config.json`
- **Проблема**: API credentials хранятся в открытом виде
- **Рекомендация**: Не делитесь этим файлом, храните приватно
